...
def workspace_autodetermine_symbols(work):
    ...
    for w in wins:
        ...
        if 'window_role' in prop:
            ...
            if role == 'browser':  tags.add('web')

        if 'class' in prop:
            clss = prop['class'].lower()

            if clss.find('firefox') != -1:        tags.add('web')
            elif clss.find('thunderbird') != -1:  tags.add('mail')
            elif clss == 'mpv':                   tags.add('film')
    # for
...
def main():
    ...
    parser.add_argument("--num", metavar='num', type=int, help="The workspace to tag")

    group = parser.add_mutually_exclusive_group()
    group.add_argument("--clear-tags", action='store_true', default=False, help="Clear all tags")
    group.add_argument("--add-tags", metavar='tag', nargs='+', type=str, help='Add a list of tags to a workspace')
    group.add_argument("--string-tags", metavar="string", type=str, help="Add an arbitrary string")
    group.add_argument("--remove-tags", metavar='tag', nargs='+', type=str, help='Remove a list of tags from a workspace')
    group.add_argument("--auto-tags", action='store_true', default=False, help="Automatically determine tags for workspace")
    group.add_argument("--set-tags", metavar='tag', nargs='+', type=str, help='Set a single tag to a workspace')
    ...
    if args.add_tags:
        ...
        try:
            ...
            unknown_tag_exit(e)
        symbols=[ s for s in symbols if not s in remsymbols ]
    elif args.set_tags:
        try:
        ...
        try:
            ...
            raise SystemExit("Could not autodetermine tags for this workspace.")
